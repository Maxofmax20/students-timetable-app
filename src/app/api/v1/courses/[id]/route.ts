...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const defaults = {
      groupId: body.groupId === undefined ? current.groupId : body.groupId,
      instructorId: body.instructorId === undefined ? current.instructorId : body.instructorId,
      roomId: body.roomId === undefined ? current.roomId : body.roomId
    };
    ...
    const preparedSessions = body.sessions?.length
      ? await prepareSessions(current.workspaceId, body.sessions, defaults)
      : (() => {
          if (body.day === undefined && body.time === undefined && body.groupId === undefined && body.instructorId === undefined && body.roomId === undefined) {
            return null;
          }

          const primarySession = current.sessions[0] ?? null;
          const normalizedDay = body.day === undefined ? primarySession?.day ?? null : normalizeDay(body.day);
          const parsedTime = body.time === undefined
            ? primarySession
              ? { startMinute: primarySession.startMinute, endMinute: primarySession.endMinute }
              : null
            : parseTimeRange(body.time);

          if (!normalizedDay || !parsedTime) return [];

          return [{
            type: (primarySession?.type ?? 'LECTURE') as 'LECTURE' | 'SECTION' | 'LAB' | 'ONLINE' | 'HYBRID',
            day: normalizedDay,
            startMinute: parsedTime.startMinute,
            endMinute: parsedTime.endMinute,
            groupId: defaults.groupId ?? null,
            instructorId: defaults.instructorId ?? null,
            roomId: (primarySession?.type ?? 'LECTURE') === 'ONLINE' ? null : defaults.roomId ?? null,
            onlinePlatform: primarySession?.onlinePlatform ?? null,
            onlineLink: primarySession?.onlineLink ?? null,
            note: primarySession?.note ?? null
          }];
        })();
    ...
    const updated = await prisma.$transaction(async (tx) => {
      await tx.course.update({
        ...
        data: {
          ...
          instructorId: defaults.instructorId,
      ...
      if (preparedSessions) {
        await tx.sessionEntry.deleteMany({ where: { courseId: id } });
        if (preparedSessions.length) {
          await tx.sessionEntry.createMany({
            data: preparedSessions.map((sessionItem) => ({

...
import { parseMinuteLabel, planningExamInclude } from '@/lib/planning';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = await prisma.exam.findMany({
      ...
      orderBy: [{ examDate: 'asc' }, { startMinute: 'asc' }],
      include: planningExamInclude
    });
    ...
    return NextResponse.json({ ok: false, message: 'EXAMS_FETCH_FAILED' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const startMinute = parseMinuteLabel(body.startTime);
    const endMinute = parseMinuteLabel(body.endTime);
    if ((startMinute == null) !== (endMinute == null)) throw new ApiError(400, 'INVALID_EXAM_TIME');
    ...
    return NextResponse.json({ ok: false, message: 'EXAM_CREATE_FAILED' }, { status: 500 });
  }
}

...
/** DELETE /api/v1/instructors/bulk — bulk delete instructors */
export async function DELETE(request: NextRequest) {
  try {
    ...
    const owned = await prisma.instructor.findMany({
    ...
    if (ownedIds.length === 0) {
      return NextResponse.json({ ok: false, message: "NO_INSTRUCTORS_FOUND" }, { status: 404 });
    }

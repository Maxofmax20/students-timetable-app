...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = await prisma.instructor.findMany({ where: { id: { in: body.ids } } });
    if (!rows.length) throw new ApiError(404, 'INSTRUCTOR_NOT_FOUND');
    ...
    for (const row of rows) {
      try {
        await prisma.$transaction(async (tx) => {
          await tx.instructor.delete({ where: { id: row.id } });
          await writeWorkspaceAudit({ tx, workspaceId, actorUserId: session.userId, entityType: 'INSTRUCTOR', entityId: row.id, actionType: 'DELETE', summary: `Bulk deleted instructor ${row.name}`, before: { name: row.name, email: row.email } });
        });
    ...
    return NextResponse.json({ ok: false, message: 'INSTRUCTOR_BULK_FAILED' }, { status: 500 });
  }
}

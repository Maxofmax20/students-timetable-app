      ...
      <div className="border-b border-[var(--border)] px-4 py-4 lg:px-5">
        <div className="flex items-center justify-between gap-2">
          <div className={cn('min-w-0', isCollapsed && 'lg:hidden')}>
            <div className="text-[11px] font-black uppercase tracking-[0.24em] text-[var(--gold)]">Students Timetable</div>
            <div className="mt-1 text-sm font-medium text-[var(--text-secondary)]">Scheduling workspace</div>
          </div>
        ...
        <div className="space-y-5">
          {navigationSections.map((section) => (
            <div key={section.title} className="space-y-1.5">
              {!isCollapsed && (
                <div className="px-3 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-muted)]">
                  {section.title}
                </div>
              )}
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        ...
                        active
                          ? 'bg-[var(--gold)] text-[var(--gold-fg)] shadow-[var(--shadow-md)]'
                          : 'text-[var(--text-secondary)] hover:bg-[var(--surface-2)] hover:text-white',
                        isCollapsed && 'lg:justify-center lg:px-0'
                      )}
                    >
                      <span className="material-symbols-outlined shrink-0 text-[20px] transition-transform group-hover:scale-110">{item.icon}</span>
